...
- your local model is mainly doing orchestration
...
- `8-10GB`: local orchestrator model plus KV headroom
...
If the box is primarily an orchestration appliance, the local router should win every resource argument. Optional image jobs, embeddings, or side experiments should not be allowed to make the control plane feel sluggish.
...
For most Moltbox builders, the real question is not "what is the best model in the abstract?" It is "what is the best local control-plane model I can run on a `16GB` card without hating the latency or the drift?"
...
- `14B Q5`: use this if the `8B` model feels too shallow and you want the smartest dense local control-plane model that still fits responsibly
...
If the model family offers an explicit "thinking" mode, the local control-plane default should usually keep it disabled. Moltbox works best when the local model behaves like a router and prompt compiler, not a deep-thinking agent.
...
For a normal Moltbox, one local control-plane model is usually the right move.
...
Do not confuse "can technically run" with "should be the control-plane default."

Judge the orchestrator by the right metrics:
