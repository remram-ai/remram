...
REM is implemented using OpenClaw runtime surfaces, not as a standalone service:

- lifecycle hooks for post-request Baseline Encode work
- plugin services for background batching and indexing
- cron/automation for Dream Machine scheduling
- tools and policy controls for any side-effecting operations
...
## 5. Baseline Encode Routine (Post-Request)
...
## 6. Dream Routine (Nightly Consolidation)
...
## 7. Clarification Loop (Morning Prompts)
...
In practice it is implemented through OpenClaw-native mechanisms:

- post-request lifecycle hooks
- scheduled cron/automation workflows
- plugin services for background consolidation work
